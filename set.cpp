...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,WF, MASS,SHAPE,LENGTH,
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else wfvalue = force->numeric(FLERR,arg[iarg+1]); //make the next argument after "workfunction" a numeric and assign to dvalue
      if (!atom->q_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      set(WF);
      iarg += 2;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (keyword == TYPE) {
      ...
      atom->q[i] = dvalue;
      //atom->workfunction[i] = wfvalue;
      }
    else if (keyword == WF) atom->workfunction[i] = wfvalue;
