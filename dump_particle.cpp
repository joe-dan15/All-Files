...
enum{X,Y,Z, // required for vtk, must come first
     ...
     IX,IY,IZ,
     VX,VY,VZ,FX,FY,FZ,
     Q, WORKFUNCTION, MUX,MUY,MUZ,MU,RADIUS,DIAMETER,
    ...
    if (iarg < narg)
    {
        ...
        for (; iarg < narg; iarg++) {
            ...
            if (strcmp(arg[iarg],"id") == 0) {
                ...
                vtype[Q] = DOUBLE;
                name[Q] = arg[iarg];
            } else if(strcmp(arg[iarg],"workfunction") == 0){
                if (!atom->q_flag)
                    error->all(FLERR,"Dumping an atom property that isn't allocated");
                pack_choice[WORKFUNCTION] = &DumpParticle::pack_workfunction;
                vtype[WORKFUNCTION] = DOUBLE;
                name[WORKFUNCTION] = arg[iarg];
    ...
    if (nthresh) {
        ...
        for (int ithresh = 0; ithresh < nthresh; ithresh++) {
            ...
            if (thresh_array[ithresh] == ID) {
                ...
                ptr = atom->q;
                nstride = 1;
            } else if (thresh_array[ithresh] == WORKFUNCTION) {
                if (!atom->q_flag)
                    error->all(FLERR,"Threshhold for an atom property that isn't allocated");
                ptr = atom->workfunction;
                nstride = 1;
...
int DumpParticle::modify_param(int narg, char **arg)
{
    ...
    if (strcmp(arg[0],"thresh") == 0) {
        ...
        else if (strcmp(arg[1],"workfunction") == 0) thresh_array[nthresh] = WORKFUNCTION;
...
void DumpParticle::pack_workfunction(int n)
{
    double *workfunction = atom->workfunction;

    for (int i = 0; i < nchoose; i++) {
        buf[n] = workfunction[clist[i]];
        n += size_one;
    }
}
